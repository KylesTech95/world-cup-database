...
echo  "$($PSQL "select name from teams as t inner join games as g on t.team_id=g.winner_id where round = 'Final' and g.year > 2017 and g.year < 2019;")"

echo -e "\nList of teams who played in the 2014 'Eighth-Final' round:"
echo "$($PSQL "select distinct(name) from teams as t inner join games as g on t.team_id=g.winner_id or t.team_id = g.opponent_id where round='Eighth-Final' and g.year=2014 order by name asc")"
